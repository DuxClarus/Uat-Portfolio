...
#===============================================================================
# #===============================================================================
# # class shieldBarrier(SpriteClass):
...
#        self.play(True)
#    
#    def toggleAmIOn(self, state = True):
#        self.amIOn = state
# #    
# #    def reverseBullet(self, bullet):
# #        print(lackoferror)
# #        #bullet.reverseYoSelfNigga!
# #    
# #    def checkEnemyBulletCollision(self, bulletList):
# #        #if(#bullet on left side check if lowerRight is past the arc REVERSE THE BULLET CAPTAIN!!! 
# #           #if on right side check the lowerRight - the width and height REVERSE THE BULLET CAPTAIN!!!!)
# #         
# #    def update(self, bulletList):
# #        if(self.amIon == True):
# #            self.checkEnemyBulletCollision(bulletList)
# #            self.draw(screen)
#===============================================================================
...
class enemyBullet(bullet):
    def __init__(self, filename, x, y, width, height, columns, rows,  xVelocity=2, yVelocity=2):
        ...
        self.bulletOffSetX = 15
        self.bulletOffSetY = 30
        self.height = 30
        self.width = 15
        self.yAtCollision = 0
    ...
    def checkPlayerCollision(self, player):
        if(self.collisionHappened == False):
            #(enemy.y + 70 >= self.y) and (enemy.y <= self.y) and (enemy.x <= self.x + self.width) and (enemy.x + enemy.width >= self.x)
            if(player.y + player.height >= self.y) and (player.y <= self.y) and (player.x <= self.x) and (player.x + player.planeOffSetX >= self.x):
    ...
    def update(self, screen, player):#, game, barrier):
        self.move()
        self.checkPlayerCollision(player)
#        if(game.sheildIsOn == True)
#            self.checkIfIHitBarrier(barrier)
        if(self.collisionHappened == True):
...
class playerBullet(bullet):
    ...
    def checkEnemyCollision(self, enemy):
        if(self.collisionHappened == False):
            if(enemy.y + 70 >= self.y) and (enemy.y <= self.y) and (enemy.x <= self.x + self.width) and (enemy.x + enemy.width >= self.x):
...
class enemyFighterJet(SpriteClass):
    ...
    def HunterKillerPather(self):
        if (self.targetLocation.x > self.x):
            ...
            self.set_animation_range(3,4)
            self.x += self.velocity.x
        if (self.targetLocation.x < self.x):
            self.set_animation_frame(1)
            self.set_animation_range(1,2)
            self.x -= self.velocity.x
    ...
    def setUpSprite(self, frame = 1, frameRangeBegin = 0, frameRangeEnd = 5, delay = 500):
        self.set_image_color_key(255,255,255)
...
class PlayerClass(SpriteClass):
    ...
    def checkPigCollision(self, game, pig):
        if(self.y - 31 <= pig.y <= self.y + 59): 
            if(self.x <= pig.x <= self.x + 15):
                game.addPoints(-1
                                )
    ...
    def update(self, screen, game, pigList):
        ...
        if(self.collisionHappened == True):
            ...
            self.toggleIsAliveFalse(game)
        for pig in pigList:
            self.checkPigCollision(game, pig)
        self.draw(screen)
...
#make enemy bullet
    ...
    def buildPlayer(self):
        #build player                                                     #-35 for the sprite offset
        self.player = PlayerClass("AirplaneSprite.bmp", (screenWidth / 2) - 35, 450, 504, 140, 7, 2, 5, 5)            
    ...
    def buildEnemies(self):
        for i in range(0,self.numberOfEnemies):
            enemy = enemyFighterJet("enemyFighterJet.bmp", random.randint(35,screenWidth-25), 0, 360, 140, 5, 2, random.randint(0,1), random.randint(0,750), -10, random.randint(2,5), random.randint(2,5))
            enemy.setUpSprite()
    ...
    def winOver(self, pressedKey):
        ...
        gameprint(screen, "Your Score: " + str(self.score), (screenWidth / 2), (screenHeight / 2), green)
...
#Toggles Pause on and off
    ...
    def gameStateRun(self):
        ...
        if(self.player.isAlive == True):
            self.player.update(screen, self, self.pigList)
        else:
            self.gameState = 3
